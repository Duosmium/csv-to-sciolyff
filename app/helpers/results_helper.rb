module ResultsHelper
  ...
  def convert_to_sciolyff(file)
    ...
    events =
      csv[1].map.with_index do |event_name, i|
        ...
        event unless event_name.nil?
      end
    events.compact!

    tracks =
      csv[3..52].take_while { |row| !row.first.nil? }.map do |row|
        track = {}
        track['name']     = row[0]
        track['medals']   = row[1] unless row[1].nil?
        track['trophies'] = row[2] unless row[2].nil?
      end

    teams =
      csv[53..1052].take_while { |row| !row.first.nil? }.map do |row|
        team = {}
    ...
    placings =
      teams.map.with_index do |team, t_i|
        events.map.with_index do |event, e_i|
          ...
          raw_place = csv[1053..2052][t_i][e_i]
  ...
  def create_name(csv)
    ...
    out += case tournament['level']
    when 'Nationals'
      "_nationals"
    when 'States'
      "_#{tournament['state']}_states"
    when 'Regionals'
      "_#{tournament['state']}_#{tournament['short name'].downcase.split('regional')[0].gsub(" ", "_")}regional"
    else
      "_#{tournament['short name'].downcase.split('invitational')[0].gsub(" ", "_")}invitational"
           end
  ...
  def gen_tournament(csv)
    ...
    tournament['start date']             = csv.first[8].nil? ? Date.parse(csv.first[7]) : Date.parse(csv.first[8])
